		...
		fetch('/EducatorConfig/api/assessment-modules', {
			...
			.then(data => {
				...
				if (data.success) {
					...
					// Add this line to update progress after module creation
					calculateAndDisplayProgress();
...
function setupSaveButton() {
	...
	const isNewAssessment = document.getElementById('is-new-assessment');
	const assessmentId = document.getElementById('assessment-id');

	if (saveBtn) {
		// Determine if this is a new assessment
		const isNew = isNewAssessment ? isNewAssessment.value === 'true' : true;

		// Update button text based on assessment state
		...
		saveBtn.addEventListener('click', function() {
			...
			const currentIsNew = isNewAssessment ? isNewAssessment.value === 'true' : true;
			const currentAssessmentId = assessmentId ? assessmentId.value : null;

			// Send data to server
			saveAssessmentData(assessmentData, currentIsNew, currentAssessmentId);
		});
	}
}
	...
	fetch(url, {
		...
		.then(data => {
			if (data.error) {
				...
				// Add this line to update progress after save
				calculateAndDisplayProgress();
				updateStatusBadge();
			}
		})
